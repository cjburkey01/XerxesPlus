...
public class ItemHelper {
	...
	public static final List<ItemStack> getDropsForBlock(World world, BlockPos pos, int fortune) {
	...
	public static List<ItemStack> getItemsFromTag(String tagName, NBTTagCompound tag) {
		NonNullList out = NonNullList.<ItemStack>create();
	...
	public static List<ItemStack> getItemsFromTag(String tagName, NBTTagCompound tag, int size) {
		NonNullList out = NonNullList.<ItemStack>withSize(size, ItemStack.EMPTY);
		if (tag == null) {
			return out;
		}
		NBTTagList items = tag.getTagList(tagName, new NBTTagCompound().getId());
	...
	public static boolean addStackToInventory(ItemStack stack, IItemHandler inventory) {
	...
	public static final boolean addStackToTileEntity(EnumFacing facing, ItemStack item, TileEntity ent) {
		if (ent == null) {
			return false;
		}
		...
		return addStackToInventory(item, itemHandler);
	}
	
	public static final boolean addStackToAdjacentInventories(ItemStack item, World world, BlockPos pos) {
		...
		if (addStackToTileEntity(EnumFacing.UP, item, world.getTileEntity(pos.down()))) {
			return true;
		}
		return false;
	}
